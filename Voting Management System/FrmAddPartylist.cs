...
namespace Voting_Management_System
{
    public partial class FrmAddPartylist : Form
    {
        ...
        #region Form Load
        private void FrmAddPartylist_Load(object sender, EventArgs e)
        {
            #region Fade In
        ...
        #endregion
        #region Edit Partylist Button
        private void BtnChangePassword_Click(object sender, EventArgs e)
        {
            try
            {
                con.Open();
                String query = "update partylist set partylist = @partylist where position_id = @id";
                MySqlCommand cmd = new MySqlCommand(query, con);
        ...
        void LogHistory(string user_id, string user_type, string user)
        {
            try
            {
                ...
                cmd3.Parameters.AddWithValue("logs", user + " has been succesfully add partylist.");
        ...
        void LogHistoryEdit(string user_id, string user_type, string user)
        {
            try
            {
                ...
                cmd3.Parameters.AddWithValue("accid", user_type);
                cmd3.Parameters.AddWithValue("user", user);
                cmd3.Parameters.AddWithValue("logs", user + " has been succesfully edit partylist.");
                cmd3.ExecuteNonQuery();
                con.Close();
            }
            catch
